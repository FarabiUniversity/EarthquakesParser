...
MILVUS_RECREATE_ON_STARTUP = os.getenv("MILVUS_RECREATE_ON_STARTUP", "false").lower() == "true"
...
def _ensure_collection_schema_matches(name: str, expected_fields: Iterable[FieldSchema]) -> None:
   ...
   if existing_sig != expected_sig:
      raise ValueError(
         "Existing Milvus collection schema does not match expected schema. "
         f"collection={name}. "
         "This usually means your Milvus volumes contain old state created by a different schema. "
         "Fix by wiping volumes or setting MILVUS_RECREATE_ON_STARTUP=true. "
         f"existing={existing_sig} expected={expected_sig}"
      )


def _validate_scalar_value(name: str, dtype: DataType, value: Any, *, max_length: int | None = None) -> None:
   """Validate a scalar field value matches the expected Milvus dtype."""
   if dtype == DataType.VARCHAR:
      if not isinstance(value, str):
         raise TypeError(f"Field '{name}' must be str (VARCHAR), got {type(value).__name__}")
      if max_length is not None and len(value) > max_length:
         raise ValueError(f"Field '{name}' exceeds max_length={max_length} (got {len(value)})")
      return

   if dtype == DataType.INT64:
      if not isinstance(value, int) or isinstance(value, bool):
         raise TypeError(f"Field '{name}' must be int (INT64), got {type(value).__name__}")
      return

   if dtype == DataType.FLOAT:
      if not isinstance(value, (int, float)) or isinstance(value, bool):
         raise TypeError(f"Field '{name}' must be float (FLOAT), got {type(value).__name__}")
...
def _validate_vector_value(name: str, value: Any, *, dim: int) -> None:
   ...
   if length != dim:
      raise ValueError(f"Field '{name}' vector dim mismatch: expected {dim}, got {length}")

   # Ensure all values are finite numbers (NaNs/Infs can cause downstream issues)
   for i, x in enumerate(value):
      if not isinstance(x, (int, float)) or isinstance(x, bool):
         raise TypeError(f"Field '{name}' vector element {i} must be float, got {type(x).__name__}")
      if not math.isfinite(float(x)):
         raise ValueError(f"Field '{name}' vector element {i} is not finite (NaN/Inf)")


def validate_record_against_collection_schema(collection_name: str, row: Dict[str, Any]) -> None:
   ...
   for field in collection.schema.fields:
      ...
      if field.dtype == DataType.FLOAT_VECTOR:
         _validate_vector_value(field.name, value, dim=int(field.params.get("dim") or getattr(field, "dim", 0)))
      else:
...
def ensure_collection_loaded(collection_name: str) -> None:
   ...
   try:
      ...
      if utility.load_state(collection_name) != LoadState.Loaded:
         raise TimeoutError(f"Collection '{collection_name}' did not reach Loaded state")
...
def create_collection_if_not_exists(
   ...
   if utility.has_collection(name):
      if MILVUS_RECREATE_ON_STARTUP:
         logger.warning(f"🧨 Dropping existing collection '{name}' due to MILVUS_RECREATE_ON_STARTUP=true")
         utility.drop_collection(name)
      else:
         # Validate schema matches what this code expects.
         _ensure_collection_schema_matches(name, fields)
         logger.info(f"⚡ Collection '{name}' already exists (schema OK; will auto-load on first use)")
