...
def test_embedding_generator_returns_1024_dim_vector():
	vec = embedding_generator.generate("Magnitude 5.2 earthquake near City X")
	assert isinstance(vec, list), "Embedding should be a list of floats"
	assert len(vec) == 1024, f"Expected 1024-d vector, got {len(vec)}"


def test_embedding_is_normalized_close_to_one():
	vec = embedding_generator.generate("Another sample text about an earthquake event")
