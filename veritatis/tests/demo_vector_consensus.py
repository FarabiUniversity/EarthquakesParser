...
def demo_centrality_scores():
    ...
    print(
        f"\n✅ Самый центральный: vec{best_idx + 1} (score={centrality_scores[best_idx]:.3f})"
    )
...
def demo_detail_scores():
    ...
    contents = [
        ...
        "Мощное землетрясение магнитудой 7.8 по шкале Рихтера произошло в Турции в понедельник утром, "
        "вызвав массовые разрушения в нескольких провинциях. Тысячи зданий обрушились, спасательные "
    ...
    print(
        f"\n✅ Самый подробный: Текст {best_idx + 1} (score={detail_scores[best_idx]:.3f})"
    )
...
def demo_combined_analysis():
    ...
    vectors = [
        ...
        },
        {
            "id": "news_3",
            "content": (
                "Разрушительное землетрясение магнитудой 7.8 по шкале Рихтера произошло в Турции "
                "в понедельник утром по местному времени. Эпицентр находился в провинции Газиантеп. "
                "Подземные толчки ощущались также в соседних странах включая Сирию, Ливан и Кипр. "
                "Тысячи зданий обрушились, особенно пострадали старые постройки. Спасательные службы "
                "работают круглосуточно, извлекая людей из-под завалов. По предварительным данным "
