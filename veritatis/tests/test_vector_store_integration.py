...
def test_insert_and_exists_and_get():
	...
	vs.create_collection_if_not_exists(
		...
		index_params={"index_type": "HNSW", "metric_type": "IP", "params": {"M": 8, "efConstruction": 32}},
	)
...
def test_insert_batch_and_move():
	...
	assert ok is True
	# Note: In Milvus v2.2, deleted records may still appear in queries until compaction.
	# So we verify the move succeeded by checking the destination, not absence from source.
