...
def analyze_tier1(
    ...
    if move_to_tier2:
        ...
        if not ids_to_move:
            ...
            moved_count = record_store.move_records(
                source_collection, target_collection, ids_to_move
            )

        print()
        print("📈 Результаты перемещения:")
        print(f"   Кандидатов (score ≥ {threshold}): {len(ids_to_move)}")
        print(f"   Перемещено в Tier 2: {moved_count}")
        print(f"   Пропущено (score < {threshold}): {skipped_count}")
        if moved_count != len(ids_to_move):
            print(
                "⚠️  Некоторые записи не были найдены в исходной коллекции "
                "(или уже перемещены ранее)."
            )
        print()
